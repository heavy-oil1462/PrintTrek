#!/usr/bin/env python3
"""PrintTrek sim container entrypoint.

Turns environment variables into a running simulated trailer:
  ...
  2. Compiles esphome/sim-trailer.yaml — the REAL firmware, esp32 target.
...
Environment:
  ...
  MQTT_ROOT=printtrek, SIM_NODE=printtrek-sim, SIM_TIMEZONE=Europe/Stockholm
...
def main() -> int:
    ...
    node = env("SIM_NODE", "printtrek-sim")
    timezone = env("SIM_TIMEZONE", "Europe/Stockholm")
    print(f"printtrek sim: node={node} broker={broker} ({broker_ip}) "
    ...
    try:
        factory = _build.compile_firmware(
            ...
            substitutions={
                ...
                "mqtt_root": env("MQTT_ROOT", "printtrek"),
            },
    ...
    print(f"printtrek sim: {exited} exited with {code}; stopping", flush=True)
    shutdown()
    for p in procs.values():
        try:
            p.wait(timeout=10)
