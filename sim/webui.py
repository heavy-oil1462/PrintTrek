#!/usr/bin/env python3
"""PrintTrek simulator web UI — drive the REAL firmware's inputs by hand.

Serves a small control panel (stdlib http.server, no dependencies beyond
paho-mqtt) that lets you set every simulated trailer input individually and
watch the firmware react:
  ...
  * Sensor injections — published retained to <root>/<node>/sim/<key>, where
    ...
    up under the real sensor ids. The water / fridge / load-shedding lambdas
    that respond are the exact C++ flashed onto real nodes.
  * Time of day — served to the firmware over SNTP. trailer-base.yaml points
    ${sntp_server} at this process (10.0.2.2 from inside QEMU), so the
    firmware's clock — and therefore the daily pump counter — follows the
  ...
  * Live entity state — everything the node publishes, straight from the
    broker, so you can see pump/lights/fridge/tier respond within seconds.

Runs inside the sim container (see sim/Containerfile) but works equally well
straight from a checkout against any broker:

    python3 sim/webui.py --broker 192.168.1.10 --username printtrek \
...
# (enforced by tools/validate.py sim).
INJECTIONS = {
...
class SimBridge:
    ...
    def __init__(self, args):
        ...
        self._lock = threading.Lock()
        self.client = mqtt_client(f"printtrek-sim-ui-{os.getpid()}",
    ...
    def _on_connect(self, client, userdata, *rest):
        ...
        if self.radio_always_on:
            # The firmware defaults to Radio Always On anyway; the retained
            # ON pins it even if a previous session switched it off.
...
PAGE = """<!doctype html>
<html><head><meta charset="utf-8"><title>printtrek simulator</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
 body{font-family:system-ui,sans-serif;margin:0;background:#12161c;color:#dbe2ea}
 header{padding:.8rem 1.2rem;background:#1a2129;border-bottom:1px solid #2c3a4a}
 h1{font-size:1.1rem;margin:0}h1 small{color:#7f95a8;font-weight:normal}
 main{display:grid;grid-template-columns:minmax(320px,430px) 1fr;gap:1rem;padding:1rem}
 @media(max-width:860px){main{grid-template-columns:1fr}}
 section{background:#1a2129;border:1px solid #2c3a4a;border-radius:8px;padding:1rem}
 h2{font-size:.85rem;text-transform:uppercase;letter-spacing:.08em;color:#7f95a8;margin:0 0 .8rem}
 .row{margin-bottom:.9rem}
 .row label{display:flex;justify-content:space-between;font-size:.85rem;margin-bottom:.2rem}
 .row output{color:#9fc0d4;font-variant-numeric:tabular-nums}
 input[type=range]{width:100%;accent-color:#4c8faf}
 button{background:#243440;color:#dbe2ea;border:1px solid #3c556a;border-radius:6px;
        padding:.35rem .7rem;margin:.15rem;cursor:pointer;font-size:.8rem}
 button:hover{background:#2f4552}
 table{width:100%;border-collapse:collapse;font-size:.82rem}
 td,th{padding:.25rem .4rem;border-bottom:1px solid #26323e;text-align:left}
 td.v{color:#9fc0d4;font-variant-numeric:tabular-nums}
 td.age{color:#5d707f;font-size:.75rem;white-space:nowrap}
 .pill{display:inline-block;padding:.1rem .55rem;border-radius:999px;font-size:.75rem}
 .on{background:#1d3d4d;color:#9fc0d4}.off{background:#4d1d1d;color:#d4a09f}
 .muted{color:#5d707f;font-size:.78rem}
 #timeview{font-size:1.3rem;color:#9fc0d4;font-variant-numeric:tabular-nums}
</style></head><body>
<header><h1>printtrek simulator <small id="conn">connecting…</small></h1></header>
<main>
<div>
...
<section><h2>Time of day</h2>
 ...
 <p class="muted">Served to the firmware over SNTP — the daily pump-runtime
 counter resets on this clock.</p></section>
...
<section><h2>Radio</h2>
 ...
 storage-mode behaviour. Injections still land (retained).</p></section>
</div>
<div>
...
def main() -> int:
    ...
    ap.add_argument("--root", default=os.environ.get("MQTT_ROOT", "printtrek"))
    ap.add_argument("--node", default=os.environ.get("SIM_NODE", "printtrek-sim"))
    ...
    print(f"printtrek simulator UI: http://0.0.0.0:{args.http_port} "
