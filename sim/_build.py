"""Shared build steps for running the PrintTrek firmware under QEMU.
...
def compile_firmware(config_dir: Path, node: str,
    ...
    """Compile sim-trailer.yaml; return the factory image path."""
    ...
    for key, value in (substitutions or {}).items():
        cmd += ["-s", key, value]
    cmd += ["compile", str(config_dir / "sim-trailer.yaml")]
