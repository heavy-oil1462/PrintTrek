{
  description = "PrintTrek - control-system devshell (ESPHome + MQTT + Home Assistant integration)";
      ...
      devShells = forAllSystems (system: pkgs:
        let
          basePackages = with pkgs; [
            ...
            mosquitto
            # tools/*.py + sim/*.py (pyyaml for validate.py's HA checks)
            (python3.withPackages (ps: with ps; [ paho-mqtt pyyaml ]))
            jq
          ];
        in
        {
