...
(esphome/sim-trailer.yaml, esp32 target) under Espressif QEMU. Sensor
values arrive via retained `printtrek/<node>/sim/<key>` topics
(packages/sim-sensors.yaml); time of day via the fake SNTP server; the
automation packages are the production ones. Engine: esphome_skills
(simc/webui/sim_build/sim_test); repo side: injections and presets in
tools/project.py, rule assertions in tools/test_sim.py.
...
docs/SIMULATION.md table, enforced by `tools/validate.py sim`. The sim
reuses the real automation packages, so entity object_ids are the same
load-bearing ids as everywhere. After touching any of this:
