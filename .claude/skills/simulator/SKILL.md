---
name: simulator
description: Run/test the REAL PrintTrek firmware without hardware — esp32 build under Espressif QEMU in a podman container against the user's own MQTT/HA, with a web UI to inject sensor values and time of day. Use for testing firmware rules end-to-end, the sim container (tools/sim_container.py), the rule integration test (tools/test_sim.py), or anything in sim/ / esphome/sim-trailer.yaml.
---

# PrintTrek simulator (real firmware under QEMU)

Full docs: `docs/SIMULATION.md`. The simulator is the actual firmware
(esphome/sim-trailer.yaml, esp32 target) under Espressif QEMU. Sensor values
arrive via retained `printtrek/<node>/sim/<key>` topics
...
sudo -E nix develop .#sim -c python3 tools/test_sim.py  # slow (~compile + minutes)
...
- `cannot bind udp/123` — SNTP port is fixed in lwIP; run as root
  ...
  free-runs (only the daily pump counter cares).
...
sim-sensors.yaml topics <-> webui.py `INJECTIONS` <-> docs/SIMULATION.md
table — enforced by `tools/validate.py sim`. The sim reuses the real
automation packages, so entity object_ids here are the same load-bearing ids
as everywhere (docs/PROTOCOL.md). Transport contract (`radio_on`/`radio_off`
scripts) is defined in trailer-base.yaml's header; radio-openeth.yaml and
