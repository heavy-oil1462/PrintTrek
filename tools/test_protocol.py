...
docs/PROTOCOL.md:
    ...
    5. direct actuator command (lights ON) -> immediate state echo
    6. ungraceful death (SIGKILL) -> broker publishes the LWT "offline"

Run inside the devshell (needs mosquitto + mosquitto_passwd + paho-mqtt):
    nix develop -c python3 tools/test_protocol.py

Exit code 0 = all assertions passed. This is part of the verification gate
and runs in CI.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="printtrek-prototest-") as tmp:
        tmp_path = Path(tmp)
        port = free_port()
        ...
        try:
            ...
            watcher.publish(f"{PREFIX}/number/pump_low_water_cutoff/command",
                            "12", retain=True)
            echo = watcher.wait_for(
                f"{PREFIX}/number/pump_low_water_cutoff/state",
                lambda v: v == "12")
            check(echo is not None, "number command echoed on state topic")

            heading("4. enable switch round trip")
            watcher.publish(f"{PREFIX}/switch/pump_enabled/command",
                            "OFF", retain=True)
            echo = watcher.wait_for(f"{PREFIX}/switch/pump_enabled/state",
                                    lambda v: v == "OFF")
            check(echo is not None, "switch command echoed on state topic")

            heading("5. direct actuator command (not retained)")
            watcher.publish(f"{PREFIX}/switch/exterior_lights/command", "ON")
            echo = watcher.wait_for(f"{PREFIX}/switch/exterior_lights/state",
                                    lambda v: v == "ON")
            check(echo is not None, "lights ON command echoed on state topic")
            watcher.publish(f"{PREFIX}/switch/exterior_lights/command", "OFF")
            echo = watcher.wait_for(f"{PREFIX}/switch/exterior_lights/state",
                                    lambda v: v == "OFF")
            check(echo is not None, "lights OFF command echoed on state topic")
            ...
            heading("6. LWT on ungraceful death")
